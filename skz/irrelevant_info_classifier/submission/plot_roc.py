...
def main():
    ...
    val_activations, val_labels = load_activations_and_labels(cache_dir, pooling_type, "val", layer_idx, device)
    ...
    train_activations, train_labels = load_activations_and_labels(cache_dir, pooling_type, "train", layer_idx, device)
...
def load_activations_and_labels(cache_dir, pooling_type, split, layer_idx, device):
    activations_path = os.path.join(cache_dir, f"{split}_{pooling_type}_activations.pt")
    labels_path = os.path.join(cache_dir, f"{split}_{pooling_type}_labels.pt")

    if not os.path.exists(activations_path) or not os.path.exists(labels_path):
        raise FileNotFoundError(
            f"Cached activations {activations_path} or labels {labels_path} not found in {cache_dir}"
        )
...
def plot_roc_curve(
    ...
    plot_filename = f"roc_curve_layer_{layer_idx}_training.png"
    plot_path = os.path.join(output_dir, plot_filename)
    plt.savefig(plot_path)
    plt.close()
    print(f"ROC curve saved to {plot_path}")

def plot_roc_curve_2(
    val_true_labels,
    val_probs,
    output_dir,
    layer_idx,
