...
class LlamaModelWrapper:
    ...
    def get_layer_output(self, layer_idx: int, input_ids: torch.Tensor, pooling_type: str) -> torch.Tensor:
        ...
        layer = outputs.hidden_states[layer_idx + 1]  # +1 to account for embeddings
        if pooling_type == "last":
            return pooling.last_pooling(layer)
        elif pooling_type == "variance":
            return pooling.variance_pooling(layer)
        elif pooling_type == "energy":
            return pooling.energy_based_pooling(layer)
        elif pooling_type == "mean":
            return pooling.mean_pooling(layer)
        elif pooling_type == "weighted":
            return pooling.weighted_pooling(layer)
        else:
...
def compute_activations(
    ...
    activations_path = os.path.join(cache_dir, f"{split_name}_{pooling_type}_activations.pt")
    labels_path = os.path.join(cache_dir, f"{split_name}_{pooling_type}_labels.pt")

    if not os.path.exists(activations_path):
        print(f"path {activations_path} does not exist")
        ...
        with torch.no_grad():
            for batch in tqdm(
                ...
                for layer_idx in layer_indices:
                    ...
                    if pooling_type == "last":
                        pooled_layer_output = pooling.last_pooling(layer_output)
                    elif pooling_type == "variance":
                        pooled_layer_output = pooling.variance_pooling(layer_output)
                    elif pooling_type == "energy":
                        pooled_layer_output = pooling.energy_based_pooling(layer_output)
                    elif pooling_type == "mean":
                        pooled_layer_output = pooling.mean_pooling(layer_output)
                    elif pooling_type == "weighted":
                        pooled_layer_output = pooling.weighted_pooling(layer_output)
                    else:
                        raise ValueError(f"Unknown pooling type: {pooling_type}")
                    # pooled_layer_output = pooling.last_pooling(layer_output)
                    ...
                    activations[layer_idx].append(pooled_layer_output.cpu())
...
def train_probes(
    ...
    for epoch in range(num_epochs):
        ...
        if accelerator.is_main_process:
            ...
            # Save metrics
            metrics_filename = f"{model_name}_{probe_type}_{hyperparams['pooling_type']}_metrics_lr{learning_rate}_epochs{num_epochs}"
            if hyperparams.get("hidden_dim"):
            ...
            for layer_idx in layer_indices:
                roc_output_path = os.path.join(
                    output_dir,
                    f"{model_name}_{probe_type}_{hyperparams['pooling_type']}_layer_{layer_idx}_lr{learning_rate}_epochs{num_epochs}",
                )
                if hyperparams.get("hidden_dim"):
...
def plot_metrics(
    ...
    for layer_idx in layer_indices:
        ...
        plot_filename = f"{model_name}_{probe_type}_{hyperparams['pooling_type']}_layer_{layer_idx}_lr{hyperparams['learning_rate']}_epochs{hyperparams['num_epochs']}"
        if hyperparams.get("hidden_dim"):
...
def plot_layer_metrics(
    ...
    plt.title(f"{model_name} {probe_type} {hyperparams['pooling_type']} Final Validation Accuracy Across Layers")
    plt.xlabel("Layer Index")
    plt.ylabel("Validation Accuracy (%)")
    plt.grid(True)
    plot_filename = f"{model_name}_{probe_type}_{hyperparams['pooling_type']}_lr{hyperparams['learning_rate']}_epochs{hyperparams['num_epochs']}_final_val_acc_across_layers.png"
    ...
    plt.title(f"{model_name} {probe_type} {hyperparams['pooling_type']} Final ROC AUC Across Layers")
    plt.xlabel("Layer Index")
    plt.ylabel("ROC AUC")
    plt.grid(True)
    roc_plot_filename = f"{model_name}_{probe_type}_{hyperparams['pooling_type']}_lr{hyperparams['learning_rate']}_epochs{hyperparams['num_epochs']}_final_roc_auc_across_layers.png"
...
def run_experiment(
    ...
    if accelerator.is_main_process:
        ...
        for i, probe in enumerate(probes):
            probe_filename = f"{model_name}_{probe_type}_{hyperparams['pooling_type']}_probe_layer_{layer_indices[i]}_lr{learning_rate}_epochs{num_epochs}"
            if hyperparams.get("hidden_dim"):
                probe_filename += f"_hd{hyperparams['hidden_dim']}"
            ...
            save_probe_with_hyperparams(
                ...
                checkpoint_path,
                {
                    ...
                    "pooling_type": hyperparams.get("pooling_type"),
                },
            )
...
def main():
    ...
    parser.add_argument(
        "--pooling_type",
        choices=["last", "variance", "energy", "mean", "weighted"],
        required=True,
        help="Choose which type of pooling to use for the probe",
    )
    ...
    # Load the dataset from a JSONL file
    # dataset_path = "datasets/distraction_clauses_dataset.jsonl"
    dataset_path = "datasets/gsm_train_with_rewritten_formatted.jsonl"
    with open(dataset_path, "r") as f:
        dataset = [json.loads(line) for line in f]
    ...
    # layer_indices6[16, 26, 31]  # Adjust based on model architecture
    ...
    output_dir = os.path.join("training_runs", f"{model_name}_{probe_type}_{args.pooling_type}_{timestamp}")
    os.makedirs(output_dir, exist_ok=True)
    ...
    results_filename = f"{model_name}_{probe_type}_{args.pooling_type}_hyperparameter_search_results.json"
